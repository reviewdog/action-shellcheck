...
if [ "${INPUT_REPORTER}" = 'github-pr-review' ]; then
  ...
  shellcheck -f json  ${INPUT_SHELLCHECK_FLAGS:-'--external-sources'} $(find "${INPUT_PATH:-'.'}" -not -path "${INPUT_EXCLUDE}" -type f -name "${INPUT_PATTERN:-'*.sh'}") \
    | jq -r '.[] | "\(.file):\(.line):\(.column):\(.level):\(.message) https://github.com/koalaman/shellcheck/wiki/SC\(.code)"' \
    | reviewdog -efm="%f:%l:%c:%t%*[^:]:%m" -name="shellcheck" -reporter="${INPUT_REPORTER:-github-pr-check}" -level="${INPUT_LEVEL}"
fi
